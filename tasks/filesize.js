...
module.exports = function (grunt) {
	...
	'use strict';
	var debug = require('debug')('grunt-filesize');
	var fs = require('fs');
	var path = require('path');
	var numeral = require('numeral');
	var moment = require('moment');
	var os = require('os');
	var defaultFormat = '{filename}: {kb,green:0,0.00} kb ({size,green:0,0} bytes)';
	...
	function mapFiles(filesSrc, options, now) {

		return filesSrc.filter(function (file) {
	...
	grunt.registerMultiTask('filesize', 'A Grunt plugin for logging file size.', function () {
		var now = new Date();
		...
		var outputs = retrieveOutputs(options.output);

		var files = mapFiles(this.filesSrc, options, now);
		...
		if (opts.reporting) {
			...
			this.files.forEach(function (file, size) {
